...
function textOp(slug:string,s:string){
 ...
 switch(slug){
  ...
  case"word-shuffler":return [...words].sort(()=>Math.random()-.5).join(" "); default:return s;
 }
}
...
function numberResult(slug:string,a:number,b:number,c:number,d:number){switch(slug){
 ...
 case"meeting-cost":return `Estimated meeting cost: ${fmt(a*b*c/60)}`; default:return"Enter values to calculate."}}
...
export default function UniversalTool({tool}:{tool:ExtraTool}){
 ...
 if(tool.kind==="number")return <div className={card}><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{[[a,setA,"Value 1"],[b,setB,"Value 2"],[c,setC,"Value 3"],[d,setD,"Value 4"]].map(([v,set,label],i)=><label key={i} className="font-bold text-[#2D241C]">{label as string}<input type="number" value={v as number} onChange={e=>(set as any)(Number(e.target.value))} className={`${input} mt-2`}/></label>)}</div><pre className="mt-6 whitespace-pre-wrap rounded-2xl bg-[#F6EBDD] p-5 text-lg font-bold text-[#5D432F]">{computed}</pre></div>;
